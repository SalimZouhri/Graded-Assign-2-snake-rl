...
class DQNNetwork(nn.Module):
    ...
    def __init__(self, board_size, n_frames, n_actions, model_config):
        ...
        for layer_name in sorted(model_config['model'].keys()):
            layer_config = model_config['model'][layer_name]
            if 'Conv2D' in layer_name:
                filters = layer_config['filters']
                kernel_size = tuple(layer_config['kernel_size'])
                padding = layer_config.get('padding', 'valid')
                padding_val = kernel_size[0] // 2 if padding == 'same' else 0
                self.conv_layers.append(nn.Conv2d(in_channels, filters, kernel_size, padding=padding_val))
                in_channels = filters
                if padding == 'valid':
                    current_size = current_size - kernel_size[0] + 1
            elif 'Dense' in layer_name:
                units = layer_config['units']
                dense_units.append(units)
        
        conv_output_size = current_size * current_size * in_channels
        dense_input = conv_output_size
        for units in dense_units:
            self.dense_layers.append(nn.Linear(dense_input, units))
            dense_input = units
        
        self.final_layer = nn.Linear(dense_input, n_actions)
...
class DeepQLearningAgent(Agent):
    ...
    def reset_models(self):
        ...
        self._model = DQNNetwork(self._board_size, self._n_frames, self._n_actions, m).to(self._device)
        if(self._use_target_net):
            self._target_net = DQNNetwork(self._board_size, self._n_frames, self._n_actions, m).to(self._device)
    ...
    def _prepare_input(self, board):
        ...
        -------
        board : torch.Tensor
            Processed and normalized board as tensor
        """
        if(board.ndim == 3):
    ...
    def save_model(self, file_path='', iteration=None):
        ...
        save function (saves in pth format)
    ...
    def load_model(self, file_path='', iteration=None):
        ...
        load function (model saved in pth format)
        ...
        if(iteration is not None):
            ...
            iteration = 0
        try:
            self._model.load_state_dict(torch.load("{}/model_{:04d}.pth".format(file_path, iteration), map_location=self._device))
            if(self._use_target_net):
                self._target_net.load_state_dict(torch.load("{}/model_{:04d}_target.pth".format(file_path, iteration), map_location=self._device))
    ...
    def train_agent(self, batch_size=32, num_games=1, reward_clip=False):
        ...
        s_tensor = self._prepare_input(s)
        next_s_tensor = self._prepare_input(next_s)
        
        current_model = self._target_net if self._use_target_net else self._model
        current_model.eval()
        with torch.no_grad():
            next_model_outputs = current_model(next_s_tensor).cpu().numpy()
        ...
        self._model.eval()
        with torch.no_grad():
            current_q = self._model(s_tensor).cpu().numpy()
        ...
        q_values = self._model(s_tensor)
        loss = mean_huber_loss(target_tensor, q_values)
        loss.backward()
    ...
    def compare_weights(self):
        ...
        for key in model_state:
            c = (model_state[key].cpu().numpy() == target_state[key].cpu().numpy()).all()
            print('Layer {:s} Match : {:d}'.format(key, int(c)))
